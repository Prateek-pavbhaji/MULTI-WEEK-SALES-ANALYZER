...
def get_weekly_sales(week_number):
    ...
    daily_sales = {}
    print(f" Enter Sales Data for WEEK {week_number} (in ₹) ---")
...
def summarize_all_weeks(all_weeks_data):
    if not all_weeks_data:
        print("No sales data entered to summarize.")
        return
    grand_total = 0
...
def main():
    ...
    while True:
        ...
        print(f">>> Week {week_counter} Total Sales: ₹{week_total:,.2f} recorded.")
        week_counter += 1
        user_input = input("Do you want to enter data for another week? (yes/no): ").strip().lower()
